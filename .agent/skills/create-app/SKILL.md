...
4.  **プロジェクトの初期化:**
    ...
    - 新しいディレクトリ内の `README.md` を更新します：
        - 1行目にメインの見出し（`# アプリ名`）を記述します。
        - その直下に、1行だけで「どんなゲームなのか、ゲームのコアとなる面白さ」を記載します。
        - それ以外の説明（概要、遊び方、基本ルールなど）は一切書かないでください。
        - 「ブラウザゲーム」といった表記は不要です（すべてブラウザゲームのため）。
5.  **最終確認:**
