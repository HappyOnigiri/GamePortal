...
4.  **プロジェクトの初期化:**
    - 新しいディレクトリ内の `app.json` を更新します：
        - `"title"` を人間が読める形式のアプリ名（例：`My New App`）に更新します。
        - `"description"` を「どんなゲームなのか、ゲームのコアとなる面白さ」が伝わる簡潔な説明に更新します。
        - `"image"` を使用するメイン画像（`src/assets/` 内のファイル名）に更新します。
