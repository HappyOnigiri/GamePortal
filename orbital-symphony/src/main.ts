import './style.css';
import { Planet, Particle, Vector } from './physics.ts';
...
class App {
  ...
  private isAddingMode = true;
  private gravityConstant = 0.5;
  ...
  private setupEventListeners() {
    ...
    document.getElementById('mode-add')?.addEventListener('click', (e) => {
      ...
      this.updateUIMode(e.target as HTMLElement);
    });
    ...
    intensitySlider?.addEventListener('input', (e) => {
      this.gravityConstant = parseFloat((e.target as HTMLInputElement).value) / 100;
    });
  }

  private updateUIMode(activeBtn: HTMLElement) {
  ...
  private removePlanet(pos: Vector) {
    const threshold = 30;
  ...
  private animate() {
    ...
    this.planets.forEach(p => {
      ...
      const grad = this.ctx.createRadialGradient(
        ...
        p.position.x, p.position.y, Math.sqrt(p.mass) * 2
      );
